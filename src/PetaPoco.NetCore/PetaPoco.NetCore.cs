...
// Define PETAPOCO_NO_DYNAMIC in your project settings on .NET 4/4.5 or NETCore1.0
...
namespace Allied.PetaPoco.NetCore
{
    ...
    public partial class Database : IDisposable
    {
        public Database(IDbConnection connection)
        {
            ...
            _providerName = connection.GetType().FullName;
            CommonConstruct();
        }
#if !COREFX
        ...
        public Database(string connectionStringName)
        {
            ...
            if (connectionStringName == "")
                connectionStringName = System.Configuration.ConfigurationManager.ConnectionStrings[0].Name;

            // Work out connection string and provider name
            var providerName = "System.Data.SqlClient";
            if (System.Configuration.ConfigurationManager.ConnectionStrings[connectionStringName] != null)
            {
                if (!string.IsNullOrEmpty(System.Configuration.ConfigurationManager.ConnectionStrings[connectionStringName].ProviderName))
        ...
        public Page<T> Page<T>(int page, int itemsPerPage, string sql, params object[] args)
        {
            ...
            result.TotalItems = ExecuteScalar<int>(sqlCount, args);
        ...
        public List<T> SkipTake<T>(int skip, int take, string sql, params object[] args)
        {
            string sqlCount, sqlPage;
        ...
        public List<T> SkipTake<T>(int skip, int take, Sql sql)
        {
        ...
        public void BulkInsertRecords<T>(IEnumerable<T> collection)
        {
            try
            {
                ...
                using (var cmd = CreateCommand(_sharedConnection, ""))
                {
                    ...
                    var cols = pd.TableInfo.AutoIncrement
                        ? string.Join(", ", pd.QueryColumns.Where(c => c.ToLower() != pd.TableInfo.PrimaryKey.ToLower()).Select(c => tableName + "." + EscapeSqlIdentifier(c)).ToArray())
                        : string.Join(", ", (from c in pd.QueryColumns select tableName + "." + EscapeSqlIdentifier(c)).ToArray());
                    ...
                    foreach (var poco in collection)
                    {
                        ...
                        foreach (var i in pd.Columns.Where(i => !pd.TableInfo.AutoIncrement || pd.TableInfo.PrimaryKey.ToLower() != i.Key.ToLower()))
                        {
        ...
        public PetaPoco.NetCore.IGridReader QueryMultiple(Sql sql)
        {
        ...
        /// <returns>A GridReader to be queried</returns>
        public PetaPoco.NetCore.IGridReader QueryMultiple(string sql, params object[] args)
        {
            OpenSharedConnection();

            PetaPoco.NetCore.GridReader result = null;
            ...
            try
            {
                ...
                result = new PetaPoco.NetCore.GridReader(this, cmd, reader, Mapper);
            }
            catch (Exception x)
            {
                OnException(x);
        ...
        public object GetAutoMapper(Type[] types)
        {
            // Build a key
        ...
        public Func<IDataReader, object, TRet> GetMultiPocoFactory<TRet>(Type[] types, string sql, IDataReader r)
        {
	...
	if (type.GetTypeInfo().IsValueType)			
        ...
        public int Save(string tableName, string primaryKeyName, object poco)
        {
            if (IsNew(primaryKeyName, poco))
            {
                return Convert.ToInt32(Insert(tableName, primaryKeyName, true, poco));
            }
            else
            {
                Type type = poco.GetType();
                var obj = type.GetProperty(primaryKeyName);
                return Update(tableName, primaryKeyName, poco) < 0 ? 0 : Convert.ToInt32(obj.GetValue(poco, null));
            }
        }

        public int Save(object poco)
        {
            var pd = PocoData.ForType(poco.GetType());
            return Save(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, poco);
        }
        ...
        public class PocoData
        {
            ...
            public PocoData(Type t)
            {
                ...
                // Get the table name
#if COREFX
		 var a = t.GetTypeInfo().GetCustomAttributes(typeof(TableNameAttribute), true);
                  TableInfo.TableName = a.Count() == 0 ? t.Name : (a.First() as TableNameAttribute).Value;
                 // Get the primary key
                a = t.GetTypeInfo().GetCustomAttributes(typeof(PrimaryKeyAttribute), true);
                TableInfo.PrimaryKey = a.Count() == 0 ? "ID" : (a.First() as PrimaryKeyAttribute).Value;
                TableInfo.SequenceName = a.Count() == 0 ? null : (a.First() as PrimaryKeyAttribute).sequenceName;
                TableInfo.AutoIncrement = a.Count() == 0 ? false : (a.First() as PrimaryKeyAttribute).autoIncrement;
...
#if COREFX
	 bool ExplicitColumns = t.GetTypeInfo().GetCustomAttributes(typeof(ExplicitColumnsAttribute), true).Count() > 0;			
#else
		...
		if (ExplicitColumns)
                    ...
                    {
                        if (pi.GetCustomAttributes(typeof(IgnoreAttribute), true).Count() != 0)
                            continue;
                    }		
#else
                    if (ExplicitColumns)
		 ...
		 if (ColAttrs.Count() > 0)
                    {
                        var colattr = (ColumnAttribute)ColAttrs.First();
...
if (type.IsValueType || type == typeof(string) || type == typeof(byte[]))
                    ...
                    else
                    {
                        // var poco=new T()
#if COREFX
                       //il.Emit(OpCodes.Newobj, type.GetTypeInfo().GetConstructor(new Type[] { Nullable.GetUnderlyingType(type) }));
		  il.Emit(OpCodes.Newobj, type.GetTypeInfo().GetConstructor(new Type[0]));
#else
...
#if COREFX
		 if (dstType.GetTypeInfo().IsEnum && IsIntegralType(srcType))		
#else
                    ...
                    else if (!dstType.IsAssignableFrom(srcType))
                    {
                        ...
                        {
                            var u = Nullable.GetUnderlyingType(dstType);
#if COREFX
		  if ((u != null) && u.GetTypeInfo().IsEnum)		
                            ...
                            {
                                if (src is string)
                                    return Enum.Parse(u, src as string);
                                return Enum.ToObject(u, src);
                            }
    ...
    public class Sql
    {
        ...
        {
            return sql != null && sql._sql != null && sql._sql.StartsWith(sqltype, StringComparison.CurrentCultureIgnoreCase);
        }
