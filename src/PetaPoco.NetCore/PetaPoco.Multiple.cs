    ...
    public class GridReader : IGridReader
    {
        ...
        private IEnumerable<T> SinglePocoFromIDataReader<T>(int index)
        {
            ...
            try
            {
                ...
                {
                    var factory =
                        pd.GetFactory(_command.CommandText, _command.Connection.ConnectionString, false, 0, _reader.FieldCount,
        ...
        private IEnumerable<TRet> MultiPocoFromIDataReader<TRet>(int index, Type[] types, object cb)
        {
            ...
            try
            {
                ...
                var factory = _db.GetMultiPocoFactory<TRet>(types, _command.CommandText, r); ;// MultiPocoFactory.GetFactory<TRet>(types, cmd.Connection.ConnectionString, cmd.CommandText, r, _defaultMapper);
                if (cb == null)
                    cb = _db.GetAutoMapper(types.ToArray());
